...
const DeleteChefAccount = ({ navigation }) => {
  ...
  const fetchDeletionInfo = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to load account information. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDeleteAccount = async () => {
    ...
    try {
      ...
      const itemsToDelete = [
        { label: 'Documents', count: deletionInfo?.documents || 0 },
        { label: 'Reviews', count: deletionInfo?.reviews || 0 },
        { label: 'Bookings', count: deletionInfo?.bookings || 0 },
        { label: 'Subscriptions', count: deletionInfo?.subscriptions || 0 },
  ...
  const navigateToScreen = (screenName) => {
    switch (screenName) {
      ...
      case 'Bookings':
        ...
        break;
      case 'Subscriptions':
        navigation.navigate('ChefProfileStatus');
        break;
    }
  };
  ...
  const renderDeletionInfo = () => {
      ...
      return (
        <View style={styles.infoBox}>
          <Text style={styles.infoText}>
            You don't have any active data that will be affected by account deletion.
          </Text>
        </View>
      );
    }
    ...
    if (deletionProgress.isDeleting) {
      return (
        <View style={styles.infoBox}>
          <Text style={[styles.infoText, { marginBottom: 15, textAlign: 'center' }]}>
              ...
              >
                ...
                <View style={styles.infoTextContainer}>
                  <Text style={[
                    ...
                    {isCompleted ? `✓ ${item.count} ${item.label} deleted` : `${item.count} ${item.label}`}
                  </Text>
                  {isCurrent && (
      ...
      <View style={styles.infoBox}>
        ...
        {items.map((item, index) => (
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Ionicons name={item.icon} size={20} color="#FF4F4F" style={styles.infoIcon} />
            <View style={styles.infoTextContainer}>
              <Text style={styles.infoText}>
...
export default DeleteChefAccount;